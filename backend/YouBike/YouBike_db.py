...
def import_data_to_db(data):
    ...
    # 插入資料的SQL語句
    insert_query = """
    INSERT INTO youbike (CityID, TownID, Name, Longitude, Latitude)
    VALUES (%s, %s, %s, %s, %s)
    """

    for item in data:
        # 假設CityID和TownID是數字，你需要為CityID和TownID對應值提供邏輯或查詢
        city_id = get_city_id(item['city'], cursor, db_connection)
        town_id = get_town_id(city_id, item['town'], cursor, db_connection)
        name = item['name']

        # 插入資料
        cursor.execute(insert_query, (city_id, town_id, name, None, None))
